...
class MIXDataParallelPPOActor(DataParallelPPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'position_ids', 'old_log_probs', 'advantages', 'rl_mask', 'sft_mask']
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    from .mix_core_alg import compute_rl_sft_loss
                    loss_fn = compute_rl_sft_loss

                    ret_dict = loss_fn(old_log_prob=old_log_prob, 
                        log_prob=log_prob,
                        advantages=advantages,
                        eos_mask=response_mask,
                        rl_mask= data['rl_mask'],
                        sft_mask= data['sft_mask'],
                    )

                    pg_loss = ret_dict['loss']
                    off_pg_loss = ret_dict['sft_loss']
