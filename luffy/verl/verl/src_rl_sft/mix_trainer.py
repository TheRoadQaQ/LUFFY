...
class MIXRayPPOTrainer(RayPPOTrainer):
    ...
    def replace_response_in_batch(self, batch, solve_none_uids):
        ...
        # sft_mask for solve_none prompts, rl_mask for other prompts
        sft_mask = torch.zeros(len(uids), dtype=torch.bool)
        rl_mask = torch.zeros(len(uids), dtype=torch.bool)

        for i,uid in enumerate(unique_uids):
            if uid in solve_none_uids:
                sft_mask[i * n] = True
            else:
                rl_mask[i*n:(i+1)*n] = True
        ...
        # replace responses for sft_mask items
        batch['responses'][sft_mask] = tgt_input_ids[sft_mask]
        
        # replace input_ids (prompt + tgt)
        batch['input_ids'][sft_mask] = torch.cat([prompts[sft_mask], tgt_input_ids[sft_mask]], dim=-1)
        ...
        batch['attention_mask'][sft_mask] = torch.cat(
            [original_attention_mask[sft_mask], response_attention_mask[sft_mask]], dim=-1)
        ...
        batch['position_ids'][sft_mask] = torch.cat(
            [original_position_ids[sft_mask], response_position_ids[sft_mask]], dim=-1)
        

        rl_mask = rl_mask.unsqueeze(1).expand(-1, response_length)
        sft_mask = sft_mask.unsqueeze(1).expand(-1, response_length)
        batch['sft_mask'] = sft_mask
        batch['rl_mask'] = rl_mask
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        on_policy_mask = batch.batch['rl_mask']
                        off_policy_mask = batch.batch['sft_mask']
...
def compute_data_metrics_ours(batch, use_critic=True):
    ...
    off_policy_mask = batch.batch['sft_mask'].any(-1) # [bsz, ]
