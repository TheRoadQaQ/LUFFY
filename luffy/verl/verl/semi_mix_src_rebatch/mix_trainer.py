...
class MIXRayPPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        sample_id2buffered_times = defaultdict(int)
            ...
            for batch_dict in self.train_dataloader:
                ...
                if not (buffer_batch is None and remaining_batch is None):
                    ...
                    if remaining_batch is not None:
                        new_batch = DataProto.concat([remaining_batch, new_batch])
                    ...
                    if buffer_batch is not None:
                        ...
                        new_batch = DataProto.concat([buffer_batch, new_batch])

                    batch = new_batch.slice(range(batch_size))
                    reorder_index = list(range(batch_size))
                    random.shuffle(reorder_index)
                    batch.reorder(torch.tensor(reorder_index))
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        for uid in unique_uids:
                            ...
                            # check rightness of off-policy guidence
                            if (uid_rewards == success_value).any():
                                if uid in solve_none_uids:
                        ...
                        seen = set()
                        unique_sample_ids = [uid for uid in sample_ids if uid not in seen and not seen.add(uid)]

                        # create buffer batch
                        # solve_non -> solve one (off-police)
                        # buffered_time < 1
                        buffer_indexes = []
                        for i, uid in enumerate(unique_uids):
                            sample_id = unique_sample_ids[i]
                            if uid in buffer_uids:
                                if sample_id2buffered_times[sample_id] < self.config.data.max_buffer_times:
                                    buffer_indexes.append(i)
                                    sample_id2buffered_times[sample_id] += 1
                            else:
                                sample_id2buffered_times[sample_id] = 0

                        if not buffer_indexes:
                            buffer_batch = None
                        else:
                            buffer_batch = batch_copy.slice(buffer_indexes)
